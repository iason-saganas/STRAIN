...
def get_real_space_values(sample_list: list | ift.ResidualSampleList):
    ...
    distributions = {
        ...
        "k0 ": ["Uniform", (100, 200)],
        # "k0 ": ["Gaussian", (50, 1e-16)],
        "p0 ": ["Gaussian", (1e3, 1e-16)],
        "c ": ["Gaussian", (100, 1e-16)],
        # "alpha ": ["Gaussian", (1, 3)],
        # "alpha ": ["Lognormal", (4, 2)],
        "alpha ": ["Exponential", (1, None)],
        # "alpha ": ["Uniform", (0, 20)],
        # "beta ": ["Gaussian", (-4, 2)],
        # "beta ": ["Negative Lognormal", (4, 4)],
        "beta ": ["Exponential", (-4, None)],
        # "beta ": ["Uniform", (0, -20)],
        "wavelet_fluct ": ["Lognormal", (4, 2)],
    ...
    for key in latent_space_values:
        ...
        if distribution_name == "Gaussian":
        ...
        elif distribution_name == "Negative Lognormal":
            x_values = [-1*np.exp(mean + sig * xi) for xi in latent_space_values[key]]
            ...
            mu = mean
            x_values = [-mu * np.log(scipy.stats.norm.cdf(xi)) for xi in latent_space_values[key]]
        else:
