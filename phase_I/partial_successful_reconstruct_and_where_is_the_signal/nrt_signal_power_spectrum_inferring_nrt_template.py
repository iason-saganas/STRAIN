...
time_domain_values_datar_space = nrt_time_values
...
s_2 = generative_model_continuous_double_power_law(h_space)
# s = generative_model(h_space)

print("k0 value: ", 2000*scipy.stats.norm.cdf(3.8711))
...
if plot_prior_samples_of_generative_model:
    ...
    if plot_power_spectrum:
        for _ in range(1):
            ...
            k_lengths_sl = h_space.get_unique_k_lengths()

            ps = s.ps
            ps_2 = s_2.ps

            xi = ift.from_random(ps.domain)
            xi_2 = ift.from_random(ps_2.domain)
            ...
            plt.plot(k_lengths_sl, sl, label="Stitched together model ")
            plt.plot(k_lengths_sl, sl_2, label="Continous model ")
            plt.loglog()

            # plt.plot(k_lengths_sl, np.abs(sl_2-sl), label="Residuals between old and new model ")
    ...
    plt.vlines(50, 2.5e-19, 100, ls="--", color="black", label=r"$k_{\mathrm{cutoff}}$")
...
posterior_samples = ift.optimize_kl(
            ...
            output_directory="outs/learning_nrt_template_continuous_double_power_law_full_k0_range  ",
