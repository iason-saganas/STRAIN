...
class NonParametricAmplitude(Model):
    ...
    def __init__(
        ...
        prefix: str = "",
        kind: str = "amplitude",
        power_spectrum_template = None,
        peak_model = None,
        make_iwp_pos_definite = False,
    ):
        ...
        :param peak_model: jft.Model | None,                A peak model, e.g. a Gaussian Comb. If this model is
                                                            endowed with methods, it is assumed that these
                                                            are functions of the class BaselineNormedGaussianComb,
                                                            and they are executed accordingly.
        :param make_iwp_pos_definite: bool,                 Whether to square the IWP part of the correlated field,
                                                            to possibly "bias" smooth baseline to rise instead of fall
                                                            at high k.
        ...
        if flexibility is not None and (log_vol.size > 0):
            ...
            self._deviations = IntegratedWienerProcess(
                ...
                asperity=asperity,
                hack_make_pos_definite=make_iwp_pos_definite,
            )
        else:
        ...
        models = [
            ...
            self._deviations,
            peak_model,
            power_spectrum_template
        ]
        domain = reduce(operator.or_, [m.domain for m in models if m is not None])

        h_grid = grid.harmonic_grid
        if power_spectrum_template is None:
            n = h_grid.mode_lengths
            power_spectrum_template = lambda p: jnp.zeros_like(n)
        if peak_model is None:
            peak_model = lambda x: 0

        self.power_spectrum_template = power_spectrum_template
        self.peak_model = peak_model

        try:
            self.finalize_peaks = self.peak_model.finalize
            # signature needs to be unweighted_peaks, ps, ps_k_values
        except AttributeError:
            # self.peak_model(primals), normed_spectrum, self.grid.harmonic_grid.mode_lengths
            self.finalize_peaks = lambda x, y, z: x  # unit in first argument, i.e. peak call itself
    ...
    def __call__(self, primals: Mapping) -> jnp.ndarray:
        ...
        if self.kind == "amplitude":
            ...
            # Built and normalize the correlated field amplitude
            ...
            # Get the correlated field power spectrum
            normed_spectrum = amplitude**2

            # Sum everything in power spectrum space, assuming the provided operators are normalized in ps-space as
            # desired, and return to amplitude space
            normed_spectrum = normed_spectrum + self.power_spectrum_template(primals)
            normed_spectrum = (normed_spectrum +
                               self.finalize_peaks(self.peak_model(primals), normed_spectrum, self.grid.harmonic_grid.mode_lengths)
                               )

            amplitude = jnp.sqrt(normed_spectrum)
            ...
            # Debug statements for your convenience:

            # import jax
            # jax.debug.print("fluctuations: = {fluc}", fluc=flu)
            # jax.debug.print("amp**2: = {amp_sq}", amp_sq=amplitude**2)
            # jax.debug.print("template: = {template}", template=self.hack_power_spectrum_template)
...
class CustomCorrelatedFieldMaker:
    ...
    def add_fluctuations(
        ...
        npa = NonParametricAmplitude(
            ...
            power_spectrum_template=power_spectrum_template,
            peak_model=peak_model,
            make_iwp_pos_definite=make_iwp_pos_definite
        )
