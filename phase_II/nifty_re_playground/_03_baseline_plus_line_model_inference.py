...
global_peak_refinement_steps = 3
pipe = pipe_1
h_xi = None  # First, we need to find a xi via penrose minimization that fits the data well
switch_to_inference_xi_in_iter = [0, 1, 1]  # if 1, h_xi gets updated to inference xi, otherwise penrose xi is recomputed
penrose_minimization_iter = [100_000, 100_000, 100_000]
...
for r in range(global_peak_refinement_steps):
    ...
    gaussian_comb_op = NormedGaussianComb(
        unique_k_lengths=pipe_1.k_signal,
        list_of_peaks=peak_pos,
        list_of_amplitudes=np.array([1e-2]*len(peak_amps)),  # set the peaks a priori on equal footing. If means here are
        # too small, some peaks will never get to explore large amplitudes
        a_priori_width_of_peaks=1,
        rel_sigma_widths=relative_std_in_peak_amplitude,
        rel_sigma_amp=relative_std_in_peak_width,
        vary_positions=False,  # The initialization will break if the peak positions are allowed to vary
        norm=True,
    )
    ...
    pipe_2.add_cfm_signal_model(add_custom_power_ops=[gaussian_comb_op, template_operator],
    ...
    if not pipe_2_called_as_import and r != 0:
        ...
        pipe_2.set_init_pos(init_pos={"xi_g_amp": latent_amplitude_init}, plot=True)
    ...
    latent_samples, vi_info = pipe_2.run_inference(kl_iterations=5, use_strict_minimizers=False,
    ...
    posterior_peak_positions = jft.mean([gaussian_comb_op.xi_g_pos_vary_or_cst(xi) for xi in latent_samples])
